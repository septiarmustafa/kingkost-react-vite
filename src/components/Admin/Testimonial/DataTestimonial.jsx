...
function DataTestimonial() {
    ...
    useEffect(() => {
        ...
        // Fetch customers
        axios.get('/customer/v1') // Adjust the endpoint based on your API
            .then(response => {
                        ...
                        <tbody>
                            {currentItems.map((review, index) => (
                                <tr key={review.id} >
                                    ...
                                    <td>{customers.find(customer => customer.id === review.customerId)?.fullName}</td>
                                    <td>{customers.find(customer => customer.id === review.customerId)?.email}</td>
                                    <td>{customers.find(customer => customer.id === review.customerId)?.address}</td>
